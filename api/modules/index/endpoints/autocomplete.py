...
@router.post("/autocomplete", response_model=autocompleteAllSearch, responses={404: {"model": Message}})
async def autocomplete_search_for_titles_across_all_modules(body: autocompleteAllSearchBody):
    items = list(filter(None, await asyncio.gather(*[call_autocomplete(module, body.text) for module in config.modules])))
    if not items:
